...
export const useTeamActivityFeed = () => {
  ...
  // Declare fetchTeamActivities BEFORE it's used
  ...
  // Use ref to store fetchTeamActivities for stable callback
  const fetchTeamActivitiesRef = useRef(fetchTeamActivities);
  fetchTeamActivitiesRef.current = fetchTeamActivities;

  // Realtime subscription using centralized manager
  const handleRealtimeChange = useCallback(() => {
    fetchTeamActivitiesRef.current();
  }, []);
