    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-body-large font-semibold">{t('teams:members')} ({activeMembers.length})</h3>
        ...
        {managers.length > 0 && (
          <div className="mb-4">
            <p className="text-caption text-muted-foreground mb-2">{t('teams:preview.teamLeaders')}</p>
            <div className="flex gap-2">
                ...
                <div key={member.id} className="flex items-center gap-2 bg-muted p-2 rounded-lg flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1">
                      {member.role === 'owner' ? <Crown className="h-3 w-3 text-primary" /> : <Shield className="h-3 w-3 text-primary" />}
                      <p className="text-[10px] text-muted-foreground">{t(`teams:roles.${member.role}`)}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {regularMembers.length > 0 && (
          <div>
            <p className="text-caption text-muted-foreground mb-2">{t('teams:members')}</p>
            ...
            {regularMembers.length > 8 && (
              <p className="text-caption text-muted-foreground text-center mt-3">
                +{regularMembers.length - 8} {t('teams:memberPlural')}
              </p>
            )}
          </div>
        )}
