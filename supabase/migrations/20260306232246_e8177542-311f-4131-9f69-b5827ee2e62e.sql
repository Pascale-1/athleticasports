-- #6: Add unique constraint for performance level upsert
ALTER TABLE player_performance_levels 
  ADD CONSTRAINT unique_performance_team_user UNIQUE (team_id, user_id);

-- #7: Add permission columns to teams
ALTER TABLE teams 
  ADD COLUMN announcement_permission text NOT NULL DEFAULT 'admin',
  ADD COLUMN chat_permission text NOT NULL DEFAULT 'member';

-- #10: Add event_id to team_announcements for linking
ALTER TABLE team_announcements 
  ADD COLUMN event_id uuid REFERENCES events(id) ON DELETE SET NULL;

-- #10: Update create_event_announcement trigger to include event_id
