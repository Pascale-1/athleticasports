...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: invitation, error: fetchError } = await supabaseServiceRole
      ...
      .select('id, team_id, status, invited_user_id, email, role, invited_by, expires_at')
      .eq('id', invitationId)
      .single();
    ...
    console.log('Invitation found:', { teamId: invitation.team_id, status: invitation.status });
    ...
    if (!isAuthorized) {
      console.log('Unauthorized acceptance attempt:', { userId: user.id, email: user.email });
      throw new Error('You are not authorized to accept this invitation');
    }

    console.log('Authorization verified:', reason);
    ...
    if (existingMember) {
      console.log('User is already a team member:', existingMember.id);
      ...
      console.log('User added to team, member_id:', memberId);
    }
    ...
    return new Response(
      JSON.stringify({ teamId: invitation.team_id }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
